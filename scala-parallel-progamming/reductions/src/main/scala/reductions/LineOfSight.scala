...
object LineOfSight {
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    (from until until).foldLeft(0F) {
      case (l, 0) =>
        l
      case (l, r) =>
        val nowTanAngle = input(r) / r
        if (nowTanAngle > l) nowTanAngle else l
    }
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    (from until until).foldLeft(startingAngle) {
      case (angle, i) =>
        if (i == 0) {
          output(0) = angle
          angle
  ...
  def parLineOfSight(input: Array[Float], output: Array[Float],
                     threshold: Int): Unit = {
    downsweep(input, output, 0f, upsweep(input, 0, input.size, threshold))
  }
}
