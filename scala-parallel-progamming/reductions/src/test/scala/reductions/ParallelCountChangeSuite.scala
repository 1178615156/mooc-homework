...
class ParallelCountChangeSuite extends FunSuite {
  ...
  test("countChange should work for multi-coins") {
    ...
    1 to 10 foreach (e =>{
      assert(parCountChange(50,List(1, 2, 5, 10),totalCoinsThreshold (e)) === 341,e)
    })
  }


  test("moneyThreshold") {
    assert(moneyThreshold(1073741823)(0, Nil) === true)
    assert(moneyThreshold(1073741823)(Int.MaxValue, Nil) === false)
  }

  test("par") {
    assert(countChange(10, List(10)) === parCountChange(10, List(10), moneyThreshold(10))
    )
  }
  test("par 2") {
    assert(countChange(16, List(1)) === parCountChange(16, List(1), moneyThreshold(16))
    )
    println(int.get())
