...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries = {
    ...
    r.minX = boundaries.minX
    r.minY = boundaries.minY
    r.maxX = boundaries.maxX
    r.maxY = boundaries.maxY
    if (body.x < r.minX) r.minX = body.x
    if (body.x > r.maxX) r.maxX = body.x
    if (body.y < r.minY) r.minY = body.x
    if (body.y > r.maxY) r.maxY = body.x
    r
  }
  ...
  def computeSectorMatrix(bodies: Seq[Body], boundaries: Boundaries): SectorMatrix = timeStats.timed("matrix") {
    val parBodies = bodies.par
    parBodies.tasksupport = taskSupport
    val r = new SectorMatrix(boundaries, SECTOR_PRECISION)
    parBodies foreach r.+=
    r
  }
  ...
  def updateBodies(bodies: Seq[Body], quad: Quad): Seq[Body] = timeStats.timed("update") {
    val parBodies = bodies.par
    parBodies.tasksupport = taskSupport
    parBodies.map(_.updated(quad)).toIndexedSeq
  }
