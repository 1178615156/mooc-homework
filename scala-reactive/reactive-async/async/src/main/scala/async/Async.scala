...
object Async {
  ...
  def sequenceComputations[A, B](
    ...
    makeAsyncComputation1().flatMap(e => makeAsyncComputation2().map(e -> _))
  ...
  def concurrentComputations[A, B](
    ...
    val a = makeAsyncComputation1()
    val b = makeAsyncComputation2()
    a.flatMap(e => b.map(e -> _))
  }
...
//  p.future
  ...
  def insist[A](makeAsyncComputation: () => Future[A], maxAttempts: Int): Future[A] =
    makeAsyncComputation().recoverWith {
      case e if maxAttempts > 1 => insist(makeAsyncComputation, maxAttempts - 1)
    }
