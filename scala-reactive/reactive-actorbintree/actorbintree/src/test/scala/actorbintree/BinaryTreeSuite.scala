...
class BinaryTreeSuite(_system: ActorSystem) extends TestKit(_system) with FunSuiteLike with Matchers with BeforeAndAfterAll with ImplicitSender {
  ...
  test("proper inserts and lookups 999") {
    val topNode = system.actorOf(Props[BinaryTreeSet])
  ...
  test("instruction example 1 ") {
    ...
    topNode ! Contains(testActor, id = 50, 2)
    expectMsg(ContainsResult(50, false))
    topNode ! Contains(testActor, id = 51, 1)
    expectMsg(ContainsResult(51, true))
  ...
  test("xxx") {
    val topNode = system.actorOf(Props[BinaryTreeSet])
    val list = List(
      (Insert(testActor, 0, 0), OperationFinished(0)),
      (Insert(testActor, 1, 0), OperationFinished(1)),
      (Insert(testActor, 2, 0), OperationFinished(2)),
      (Remove(testActor, 3, 0), OperationFinished(3)),
      (Insert(testActor, 4, 0), OperationFinished(4)),
      (Insert(testActor, 5, 0), OperationFinished(5)),
      (Insert(testActor, 6, 0), OperationFinished(6)),
      (Remove(testActor, 7, 0), OperationFinished(7)),
      (Remove(testActor, 8, 0), OperationFinished(8)),
      (Insert(testActor, 9, 0), OperationFinished(9)),
      (Insert(testActor, 10, 0), OperationFinished(10)),
      (Remove(testActor, 11, 0), OperationFinished(11)),
      (Contains(testActor, 12, 0), ContainsResult(12, false))
    )
  }
  test("instruction example 2 ") {
    ...
    List(1, 6, 2, 3, 4, 3, 2, 1).foreach { n =>
      println(n)

      topNode ! Contains(testActor, id = 2, n)
      expectMsg(ContainsResult(2, false))
      topNode ! Insert(testActor, id = 3, n)
      expectMsg(OperationFinished(3))
      topNode ! Contains(testActor, id = 4, n)
      expectMsg(ContainsResult(4, true))

      topNode ! Remove(testActor, id = 1, n)
  ...
  test("behave identically to built-in set ") {
    val rnd = new Random()

    def randomOperations(requester: ActorRef, count: Int): Seq[Operation] = {
      def randomElement: Int = rnd.nextInt(1)
    ...
    ops.zip(expectedReplies).foreach(println)
