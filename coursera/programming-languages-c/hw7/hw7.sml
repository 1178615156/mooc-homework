...
fun eval_prog (e,env) =
    case e of
	  ...
	  (* p2 Shift*)
	  | Shift (dx,dy,geom) =>
		case eval_prog(geom,env) of  
			NoPoints => NoPoints
		  |	Point(x,y) => Point(x + dx , y + dy )
		  | Line(m,b)  => Line(m , b + dy - m * dx )
		  | VerticalLine(x) => VerticalLine(x + dx)
		  | LineSegment(ax,ay,bx,by) =>  LineSegment(ax+dx,ay+dy,bx+dx,by+dy)

(* p1 *)
fun preprocess_prog e = case e of 
	  LineSegment(ax,ay,bx,by)=> 
		if real_close_point(ax,ay)(bx,by) then Point(ax,ay)
		else if ax < bx then e 
		else if real_close(ax,bx) andalso ay < by then e 
		else LineSegment(bx,by,ax,ay)
	| Let(s,e1,e2) 		=> Let(s, preprocess_prog(e1), preprocess_prog(e2))
	| Intersect(e1,e2)	=> Intersect(preprocess_prog(e1), preprocess_prog(e2))
	| Shift (dx,dy,geom)=> Shift(dx,dy, preprocess_prog(geom))
	| _ => e