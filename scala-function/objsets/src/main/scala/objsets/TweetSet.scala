...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def descendingByRetweet: TweetList ={
   val set =  left union right
    var list:TweetList = Nil
    set.filter(_.retweets <= elem.retweets).descendingByRetweet
      .foreach(e=>{
        list = new Cons(e,list)
      })
    list = new Cons(elem,list)
    set.filter(_.retweets > elem.retweets).descendingByRetweet
      .foreach(e=>{
        list = new Cons(e,list)
      })
  ...
  override def union(that: TweetSet): TweetSet ={
    ...
    new NonEmpty(elem,l,that )

  }

  override def mostRetweeted: Tweet = {
    var i :Tweet = null
    descendingByRetweet.foreach(e=>{
      i = e
    })
    i
  }
