...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    rdd.aggregate(0)(
      seqOp = (acc, article) => acc + (if(article.text.split(" ").contains(lang)) 1 else 0),
      combOp = _ + _
    )
  }
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    sc.makeRDD(rdd.aggregate(langs.map(_ -> List[WikipediaArticle]()))(
      seqOp = (langs, article) => {
        val text = article.text.split(" ")
        langs.map { case (lang, docs) => lang -> (if(text.contains(lang)) article +: docs else docs) }
      }
      ,
      combOp = (l, r) => (l ++ r).groupBy(_._1).mapValues(_.flatMap(_._2)).toList
    ))
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    rdd.flatMap(e => e.text.split(" ").toList.filter(langs.contains(_)).distinct)
      .map(_ -> 1)
      .reduceByKey(_ + _)
      .collect().toList.sortBy(-_._2)
  }
